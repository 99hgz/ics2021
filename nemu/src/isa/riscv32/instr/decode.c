...
static def_DHelper(B) {
  ...
  sword_t simm = SEXT32((s->isa.instr.b.imm4_1 << 1) | (s->isa.instr.b.imm11 << 11) | (s->isa.instr.b.imm10_5 << 5) | (s->isa.instr.b.imm12 << 12),13);
  //printf("B-type imm=%d\n",simm);
...
static def_DHelper(I2) {
  decode_op_r(s, id_src1, s->isa.instr.i.rs1, false);
  decode_op_r2(s, id_src2, s->isa.instr.i.simm11_0, false);
  decode_op_r(s, id_dest, s->isa.instr.i.rd, true);
}

static def_DHelper(I3) {
  decode_op_i(s, id_src1, ZEXT32(s->isa.instr.i.rs1,5), false);
  decode_op_r2(s, id_src2, s->isa.instr.i.simm11_0, false);
  decode_op_r(s, id_dest, s->isa.instr.i.rd, true);
}
...
def_THelper(main) {
  ...
  def_INSTR_IDTAB("??????? ????? ????? 000 ????? 11100 11", I      , csrf);
  def_INSTR_IDTAB("??????? ????? ????? 0?? ????? 11100 11", I2     , icsr);
  def_INSTR_IDTAB("??????? ????? ????? 1?? ????? 11100 11", I3     , icsri);
